...
func (s *networkInterfaceClaimStrategy) Adopt(ctx context.Context, claimer client.Object, obj client.Object) error {
	nic := obj.(*networkingv1beta1.NetworkInterface)
	base := nic.DeepCopy()
	nic.Spec.MachineRef = commonv1beta1.NewLocalObjUIDRef(claimer)
	nic.Spec.ProviderID = ""
...
func (r *MachineReconciler) networkInterfaceNameToMachineNetworkInterfaceName(machine *computev1beta1.Machine) map[string]string {
	sel := make(map[string]string)
	for _, machineNic := range machine.Spec.NetworkInterfaces {
		nicName := computev1beta1.MachineNetworkInterfaceName(machine.Name, machineNic)
		sel[nicName] = machineNic.Name
	}
	return sel
}

func (r *MachineReconciler) machineNetworkInterfaceSelector(machine *computev1beta1.Machine) claimmanager.Selector {
	names := sets.New(computev1beta1.MachineNetworkInterfaceNames(machine)...)
...
func (r *MachineReconciler) getNetworkInterfacesForMachine(ctx context.Context, machine *computev1beta1.Machine) ([]networkingv1beta1.NetworkInterface, error) {
...
func (r *MachineReconciler) prepareORINetworkInterfacesForMachine(
	...
	nics []networkingv1beta1.NetworkInterface,
) ([]*ori.NetworkInterface, map[string]v1beta1.ObjectUIDRef, bool, error) {
	...
	if len(oriNics) != len(machine.Spec.Volumes) {
		expectedNicNames := utilslices.ToSetFunc(machine.Spec.NetworkInterfaces, func(v computev1beta1.NetworkInterface) string { return v.Name })
...
func (r *MachineReconciler) getORINetworkInterfacesForMachine(
	...
	machine *computev1beta1.Machine,
	nics []networkingv1beta1.NetworkInterface,
) ([]*ori.NetworkInterface, map[string]v1beta1.ObjectUIDRef, error) {
	var (
		nicNameToMachineNicName = r.networkInterfaceNameToMachineNetworkInterfaceName(machine)

		oriNics                []*ori.NetworkInterface
		machineNicNameToUIDRef = make(map[string]v1beta1.ObjectUIDRef)
		errs                   []error
	)
	for _, nic := range nics {
		...
		machineNicNameToUIDRef[machineNicName] = v1beta1.ObjUID(&nic)
	}
...
func (r *MachineReconciler) getNetworkInterfaceIP(
	...
	case nicIP.Ephemeral != nil:
		...
		if prefix.Status.Phase != ipamv1beta1.PrefixPhaseAllocated {
	...
	default:
		return commonv1beta1.IP{}, false, fmt.Errorf("unrecognized network interface ip %#v", nicIP)
	}
}
...
func (r *MachineReconciler) prepareORINetworkInterface(
	...
	return &ori.NetworkInterface{
		...
		Ips:        utilslices.Map(ips, commonv1beta1.IP.String),
...
func (r *MachineReconciler) computeNetworkInterfaceMapping(
	...
	nics []networkingv1beta1.NetworkInterface,
	oriNics []*ori.NetworkInterface,
) map[string]v1beta1.ObjectUIDRef {
	nicByName := utilslices.ToMapByKey(nics,
		func(nic networkingv1beta1.NetworkInterface) string { return nic.Name },
	)
	...
	nicMapping := make(map[string]v1beta1.ObjectUIDRef, len(oriNics))
	for _, oriNic := range oriNics {
		nicName := machineNicNameToNicName[oriNic.Name]
		nic := nicByName[nicName]

		nicMapping[oriNic.Name] = v1beta1.ObjUID(&nic)
	}
	return nicMapping
}

var oriNetworkInterfaceStateToNetworkInterfaceState = map[ori.NetworkInterfaceState]computev1beta1.NetworkInterfaceState{
	ori.NetworkInterfaceState_NETWORK_INTERFACE_PENDING:  computev1beta1.NetworkInterfaceStatePending,
	ori.NetworkInterfaceState_NETWORK_INTERFACE_ATTACHED: computev1beta1.NetworkInterfaceStateAttached,
}

func (r *MachineReconciler) convertORINetworkInterfaceState(state ori.NetworkInterfaceState) (computev1beta1.NetworkInterfaceState, error) {
...
func (r *MachineReconciler) convertORINetworkInterfaceStatus(status *ori.NetworkInterfaceStatus) (computev1beta1.NetworkInterfaceStatus, error) {
	state, err := r.convertORINetworkInterfaceState(status.State)
	if err != nil {
		return computev1beta1.NetworkInterfaceStatus{}, err
	}

	return computev1beta1.NetworkInterfaceStatus{
		Name:   status.Name,
...
func (r *MachineReconciler) addNetworkInterfaceStatusValues(now metav1.Time, existing, newValues *computev1beta1.NetworkInterfaceStatus) {
...
func (r *MachineReconciler) getNetworkInterfaceStatusesForMachine(
	...
	oriMachine *ori.Machine,
	now metav1.Time,
) ([]computev1beta1.NetworkInterfaceStatus, error) {
	var (
		oriNicStatusByName        = utilslices.ToMapByKey(oriMachine.Status.NetworkInterfaces, (*ori.NetworkInterfaceStatus).GetName)
		existingNicStatusesByName = utilslices.ToMapByKey(machine.Status.NetworkInterfaces, func(status computev1beta1.NetworkInterfaceStatus) string { return status.Name })
		nicStatuses               []computev1beta1.NetworkInterfaceStatus
		errs                      []error
	)
	...
	for _, machineNic := range machine.Spec.NetworkInterfaces {
		var (
			...
			nicStatusValues  computev1beta1.NetworkInterfaceStatus
		)
		if ok {
			var err error
			...
			nicStatusValues = computev1beta1.NetworkInterfaceStatus{
				...
				State: computev1beta1.NetworkInterfaceStatePending,
			}
		}
