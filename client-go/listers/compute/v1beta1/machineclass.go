...
type MachineClassLister interface {
	...
	List(selector labels.Selector) (ret []*v1beta1.MachineClass, err error)
	// Get retrieves the MachineClass from the index for a given name.
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1beta1.MachineClass, error)
...
// List lists all MachineClasses in the indexer.
func (s *machineClassLister) List(selector labels.Selector) (ret []*v1beta1.MachineClass, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1beta1.MachineClass))
	})
	return ret, err
}

// Get retrieves the MachineClass from the index for a given name.
func (s *machineClassLister) Get(name string) (*v1beta1.MachineClass, error) {
	obj, exists, err := s.indexer.GetByKey(name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1beta1.Resource("machineclass"), name)
	}
	return obj.(*v1beta1.MachineClass), nil
}
