...
type MachinePoolLister interface {
	...
	List(selector labels.Selector) (ret []*v1beta1.MachinePool, err error)
	// Get retrieves the MachinePool from the index for a given name.
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1beta1.MachinePool, error)
...
// List lists all MachinePools in the indexer.
func (s *machinePoolLister) List(selector labels.Selector) (ret []*v1beta1.MachinePool, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1beta1.MachinePool))
	})
	return ret, err
}

// Get retrieves the MachinePool from the index for a given name.
func (s *machinePoolLister) Get(name string) (*v1beta1.MachinePool, error) {
	obj, exists, err := s.indexer.GetByKey(name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1beta1.Resource("machinepool"), name)
	}
	return obj.(*v1beta1.MachinePool), nil
}
