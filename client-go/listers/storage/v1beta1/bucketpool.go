...
type BucketPoolLister interface {
	...
	List(selector labels.Selector) (ret []*v1beta1.BucketPool, err error)
	// Get retrieves the BucketPool from the index for a given name.
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1beta1.BucketPool, error)
...
// List lists all BucketPools in the indexer.
func (s *bucketPoolLister) List(selector labels.Selector) (ret []*v1beta1.BucketPool, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1beta1.BucketPool))
	})
	return ret, err
}

// Get retrieves the BucketPool from the index for a given name.
func (s *bucketPoolLister) Get(name string) (*v1beta1.BucketPool, error) {
	obj, exists, err := s.indexer.GetByKey(name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1beta1.Resource("bucketpool"), name)
	}
	return obj.(*v1beta1.BucketPool), nil
}
