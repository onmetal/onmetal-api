...
type VolumeClassLister interface {
	...
	List(selector labels.Selector) (ret []*v1beta1.VolumeClass, err error)
	// Get retrieves the VolumeClass from the index for a given name.
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1beta1.VolumeClass, error)
...
// List lists all VolumeClasses in the indexer.
func (s *volumeClassLister) List(selector labels.Selector) (ret []*v1beta1.VolumeClass, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1beta1.VolumeClass))
	})
	return ret, err
}

// Get retrieves the VolumeClass from the index for a given name.
func (s *volumeClassLister) Get(name string) (*v1beta1.VolumeClass, error) {
	obj, exists, err := s.indexer.GetByKey(name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1beta1.Resource("volumeclass"), name)
	}
	return obj.(*v1beta1.VolumeClass), nil
}
