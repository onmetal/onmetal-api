...
type BucketClassLister interface {
	...
	List(selector labels.Selector) (ret []*v1beta1.BucketClass, err error)
	// Get retrieves the BucketClass from the index for a given name.
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1beta1.BucketClass, error)
...
// List lists all BucketClasses in the indexer.
func (s *bucketClassLister) List(selector labels.Selector) (ret []*v1beta1.BucketClass, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1beta1.BucketClass))
	})
	return ret, err
}

// Get retrieves the BucketClass from the index for a given name.
func (s *bucketClassLister) Get(name string) (*v1beta1.BucketClass, error) {
	obj, exists, err := s.indexer.GetByKey(name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1beta1.Resource("bucketclass"), name)
	}
	return obj.(*v1beta1.BucketClass), nil
}
