...
type VolumePoolLister interface {
	...
	List(selector labels.Selector) (ret []*v1beta1.VolumePool, err error)
	// Get retrieves the VolumePool from the index for a given name.
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1beta1.VolumePool, error)
...
// List lists all VolumePools in the indexer.
func (s *volumePoolLister) List(selector labels.Selector) (ret []*v1beta1.VolumePool, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1beta1.VolumePool))
	})
	return ret, err
}

// Get retrieves the VolumePool from the index for a given name.
func (s *volumePoolLister) Get(name string) (*v1beta1.VolumePool, error) {
	obj, exists, err := s.indexer.GetByKey(name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1beta1.Resource("volumepool"), name)
	}
	return obj.(*v1beta1.VolumePool), nil
}
