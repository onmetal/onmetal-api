...
type NATGatewayLister interface {
	...
	List(selector labels.Selector) (ret []*v1beta1.NATGateway, err error)
...
// List lists all NATGateways in the indexer.
func (s *nATGatewayLister) List(selector labels.Selector) (ret []*v1beta1.NATGateway, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1beta1.NATGateway))
	})
	return ret, err
}
...
type NATGatewayNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1beta1.NATGateway, error)
...
// List lists all NATGateways in the indexer for a given namespace.
func (s nATGatewayNamespaceLister) List(selector labels.Selector) (ret []*v1beta1.NATGateway, err error) {
...
// Get retrieves the NATGateway from the indexer for a given namespace and name.
func (s nATGatewayNamespaceLister) Get(name string) (*v1beta1.NATGateway, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1beta1.Resource("natgateway"), name)
	}
	return obj.(*v1beta1.NATGateway), nil
}
