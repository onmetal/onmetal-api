...
type PrefixLister interface {
	...
	List(selector labels.Selector) (ret []*v1beta1.Prefix, err error)
...
// List lists all Prefixes in the indexer.
func (s *prefixLister) List(selector labels.Selector) (ret []*v1beta1.Prefix, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1beta1.Prefix))
	})
	return ret, err
}
...
type PrefixNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1beta1.Prefix, error)
...
// List lists all Prefixes in the indexer for a given namespace.
func (s prefixNamespaceLister) List(selector labels.Selector) (ret []*v1beta1.Prefix, err error) {
...
// Get retrieves the Prefix from the indexer for a given namespace and name.
func (s prefixNamespaceLister) Get(name string) (*v1beta1.Prefix, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1beta1.Resource("prefix"), name)
	}
	return obj.(*v1beta1.Prefix), nil
}
