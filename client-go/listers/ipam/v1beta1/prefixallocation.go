...
type PrefixAllocationLister interface {
	...
	List(selector labels.Selector) (ret []*v1beta1.PrefixAllocation, err error)
...
// List lists all PrefixAllocations in the indexer.
func (s *prefixAllocationLister) List(selector labels.Selector) (ret []*v1beta1.PrefixAllocation, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1beta1.PrefixAllocation))
	})
	return ret, err
}
...
type PrefixAllocationNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1beta1.PrefixAllocation, error)
...
// List lists all PrefixAllocations in the indexer for a given namespace.
func (s prefixAllocationNamespaceLister) List(selector labels.Selector) (ret []*v1beta1.PrefixAllocation, err error) {
...
// Get retrieves the PrefixAllocation from the indexer for a given namespace and name.
func (s prefixAllocationNamespaceLister) Get(name string) (*v1beta1.PrefixAllocation, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1beta1.Resource("prefixallocation"), name)
	}
	return obj.(*v1beta1.PrefixAllocation), nil
}
